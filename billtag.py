...
def main():
    ...
    ap.add_argument('-d', '--delivery', default=0, type=parse_decimal, help='add total delivery (in foreign currency), weighted by line total')
    args = ap.parse_args()
    with open(args.input) as infp:
        data = list(read_tsv(infp))
    processed = process(data, rounding=args.rounding, currency_multiplier=args.currency_multiplier, delivery=args.delivery)
